...
void nr_fill_pucch(PHY_VARS_gNB *gNB,
  ...
  if (pucch->pucch_pdu.format_type > 0) {
    LOG_D(NR_PHY,"Programming PUCCH[%d] for %d.%d, format %d, nb_harq %d, nb_sr %d, nb_csi %d\n",id,
...
void nr_decode_pucch0(PHY_VARS_gNB *gNB,
  ...
  LOG_D(NR_PHY,"pucch0: nr_symbols %d, start_symbol %d, prb_start %d, second_hop_prb %d,  group_hop_flag %d, sequence_hop_flag %d, O_ACK %d, O_SR %d, mcs %d initial_cyclic_shift %d\n",
  ...
  if (pucch_pdu->freq_hop_flag == 1) {
    ...
    LOG_D(NR_PHY,"pucch0 second hop: u %d, v %d\n",u[1],v[1]);
  ...
  for(i=0;i<nr_sequences;i++){
    ...
    if (temp>xrtmag) {
      xrtmag_next = xrtmag;
      xrtmag=temp;
      LOG_D(NR_PHY,"Sequence %d xrtmag %ld xrtmag_next %ld\n", i, xrtmag, xrtmag_next);
      maxpos=i;
  ...
  if (pucch_pdu->bit_len_harq==0) {
    ...
    if (xrtmag_dBtimes10>(10*max_n0+100)) {
      ...
      LOG_D(NR_PHY,"PUCCH0 got positive SR. Cumulative number of positive SR %d\n", uci_stats->pucch0_positive_SR);
    } else {
    ...
    if (no_conf) LOG_I(NR_PHY, "[DLSCH/PDSCH/PUCCH] %d.%d HARQ values (%s, %s) with confidence level %s, xrt_mag %d xrt_mag_next %d n0 %d (%d,%d) pucch0_thres %d, cqi %d, SNRtimes10 %d,sync_pos %d\n",
          frame,slot,
...
void nr_decode_pucch2(PHY_VARS_gNB *gNB,
  ...
  LOG_D(NR_PHY,"Decoding pucch2 for %d symbols, %d PRB, nb_harq %d, nb_sr %d, nb_csi %d/%d\n",
...
#endif
            LOG_D(NR_PHY,"pucch2 cw %d group %d aa %d: (%d,%d)+(%d,%d) = (%d,%d)\n",cw,group,aa,
              corr32_re[symb][group][aa],corr32_im[symb][group][aa],
              ((int16_t*)(&prod_re[aa]))[0],
              ((int16_t*)(&prod_im[aa]))[0],
              corr32_re[symb][group][aa]+((int16_t*)(&prod_re[aa]))[0],
...
#endif
    LOG_D(NR_PHY,"slot %d PUCCH2 cw_ML %d, metric %d dB\n",slot,cw_ML,corr_dB);
      ...
      for (int half_prb=0;half_prb<(2*pucch_pdu->prb_size);half_prb++) {
        ...
        for (int cw=0;cw<256;cw++) {
          ...
          for (int aa=0;aa<Prx;aa++) {
            ...
            corr_tmp += corr_re*corr_re + corr_im*corr_im;
           /*
              LOG_D(NR_PHY,"pucch2 half_prb %d cw %d (%d,%d) aa %d: (%d,%d,%d,%d,%d,%d,%d,%d)x(%d,%d,%d,%d,%d,%d,%d,%d)  (%d,%d)+(%d,%d) = (%d,%d) => %d\n",
...
*/
	...
	LOG_D(NR_PHY,"half_prb %d cw %d corr16 %d\n",half_prb,cw,corr_tmp>>8);
	...
	LOG_D(NR_PHY,"lut_num (%d,%d,%d,%d,%d,%d,%d,%d)\n",
	...
	LOG_D(NR_PHY,"llr_num (%d,%d,%d,%d,%d,%d,%d,%d)\n",
	      ((int16_t*)&llr_num)[0],
	      ((int16_t*)&llr_num)[1],
	      ((int16_t*)&llr_num)[2],
	      ((int16_t*)&llr_num)[3],
	      ...
	      ((int16_t*)&llr_num)[6],
	      ((int16_t*)&llr_num)[7]);
	LOG_D(NR_PHY,"llr_den (%d,%d,%d,%d,%d,%d,%d,%d)\n",
	      ((int16_t*)&llr_den)[0],
	      ((int16_t*)&llr_den)[1],
	      ((int16_t*)&llr_den)[2],
	      ((int16_t*)&llr_den)[3],
      ...
      // compute llrs
        ...
        LOG_D(NR_PHY,"llrs[%d] : (%d,%d,%d,%d,%d,%d,%d,%d)\n",
              half_prb,
    ...
    LOG_D(NR_PHY,"UCI decoderState %d, payload[0] %llu\n",decoderState,(unsigned long long)decodedPayload[0]);
    if (decoderState>0) decoderState=1;
    corr_dB = dB_fixed64(corr);
    LOG_D(NR_PHY,"metric %d dB\n",corr_dB);
  }
