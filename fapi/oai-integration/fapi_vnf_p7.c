...
int aerial_phy_unpack_p7_vendor_extension(nfapi_p7_message_header_t *header, uint8_t **ppReadPackedMessage, uint8_t *end, nfapi_p7_codec_config_t *config) {
...
int aerial_phy_pack_p7_vendor_extension(nfapi_p7_message_header_t *header, uint8_t **ppWritePackedMsg, uint8_t *end, nfapi_p7_codec_config_t *config) {
  //NFAPI_TRACE(NFAPI_TRACE_INFO, "%s\n", __FUNCTION__);
...
int aerial_phy_unpack_vendor_extension_tlv(nfapi_tl_t *tl, uint8_t **ppReadPackedMessage, uint8_t *end, void **ve, nfapi_p7_codec_config_t *codec) {
  (void)tl;
...
uint8_t aerial_unpack_nr_slot_indication(uint8_t **ppReadPackedMsg, uint8_t *end, nfapi_nr_slot_indication_scf_t *msg, nfapi_p7_codec_config_t* config){
  return  unpack_nr_slot_indication(ppReadPackedMsg, end, msg,config);
}

uint8_t aerial_unpack_nr_rx_data_indication(uint8_t **ppReadPackedMsg, uint8_t *end, nfapi_nr_rx_data_indication_t *msg, nfapi_p7_codec_config_t* config){
  return unpack_nr_rx_data_indication(ppReadPackedMsg,end, msg, config);
}

uint8_t aerial_unpack_nr_crc_indication(uint8_t **ppReadPackedMsg, uint8_t *end, nfapi_nr_crc_indication_t *msg, nfapi_p7_codec_config_t* config){
...
uint8_t aerial_unpack_nr_rach_indication(uint8_t **ppReadPackedMsg, uint8_t *end, nfapi_nr_rach_indication_t* msg, nfapi_p7_codec_config_t* config){
...
int fapi_nr_p7_message_pack(void *pMessageBuf, void *pPackedBuf, uint32_t packedBufLen, nfapi_p7_codec_config_t* config)
{
  ...
  // Message Body [8,...]
  if( !(push8(1, &pWritePackedMessage, pPackMessageEnd) &&
        push8(0, &pWritePackedMessage, pPackMessageEnd) &&
  ...
  switch (pMessageHeader->message_id)
  {
    ...
    case NFAPI_NR_PHY_MSG_TYPE_TX_DATA_REQUEST:
      result = pack_tx_data_request(pMessageHeader, &pPacketBodyField, end, config);
      break;
...
int fapi_nr_pack_and_send_p7_message(vnf_p7_t* vnf_p7, nfapi_p7_message_header_t* header){
  ...
  printf(" msg id : 0x%02x length: %d\n", header->message_id, len_FAPI);
  test_nv_ipc_send_msg_P7(FAPI_buffer, len_FAPI, header);
