    ...
    if (nfapi_p5_message_unpack(pRecvMsg, recvMsgLen, &msg, sizeof(msg), &config->codec_config) >= 0) {
      ...
      if (msg.error_code == NFAPI_MSG_OK) {
        ...
        if (msg.nfapi_config.p7_pnf_address_ipv4.tl.tag) {
          ...
          (void) memcpy(&sockAddr.sin_addr.s_addr, msg.nfapi_config.p7_pnf_address_ipv4.address,
...
void vnf_nr_handle_start_response(void *pRecvMsg, int recvMsgLen, nfapi_vnf_config_t *config, int p5_idx) {
  ...
  if (pRecvMsg == NULL || config == NULL) {
    ...
    // unpack the message
    if (nfapi_nr_p5_message_unpack(pRecvMsg, recvMsgLen, &msg, sizeof(msg), &config->codec_config) >=
        0) {  // check the error code
...
void vnf_handle_system_information_schedule_response(void *pRecvMsg, int recvMsgLen, nfapi_vnf_config_t *config,
                                                     int p5_idx) {
  // ensure it's valid
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
...
void vnf_handle_system_information_schedule_indication(void *pRecvMsg, int recvMsgLen, nfapi_vnf_config_t *config,
                                                       int p5_idx) {
  // ensure it's valid
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
...
void vnf_handle_nmm_stop_response(void *pRecvMsg, int recvMsgLen, nfapi_vnf_config_t *config, int p5_idx) {
  ...
  if (pRecvMsg == NULL || config == NULL) {
    ...
    // unpack the message
    if (nfapi_p4_message_unpack(pRecvMsg, recvMsgLen, &msg, sizeof(nfapi_nmm_stop_response_t), &config->codec_config) >=
        0) {
...
void vnf_handle_vendor_extension(void *pRecvMsg, int recvMsgLen, nfapi_vnf_config_t *config, int p5_idx,
                                 uint16_t message_id) {
...
void vnf_nr_handle_p4_p5_message(void *pRecvMsg, int recvMsgLen, int p5_idx, nfapi_vnf_config_t *config) {
  ...
  if (nfapi_p5_message_header_unpack(pRecvMsg, recvMsgLen, &messageHeader, sizeof(nfapi_p4_p5_message_header_t),
                                     &config->codec_config) < 0) {
...
int vnf_nr_read_dispatch_message(nfapi_vnf_config_t *config, nfapi_vnf_pnf_info_t *pnf) {
  if (1) {
    ...
    {
      int flags = MSG_PEEK;
      message_size = sctp_recvmsg(pnf->p5_sock, header_buffer, header_buffer_size, (struct sockaddr *) &addr, &addr_len,
                                  &sndrcvinfo, &flags);
...

    {
      int flags = 0;
      (void) memset(&sndrcvinfo, 0, sizeof(struct sctp_sndrcvinfo));
      printf("VNF Received message with size: %d\n", message_size);
      int recvmsg_result = sctp_recvmsg(pnf->p5_sock, read_buffer, message_size, (struct sockaddr *) &addr, &addr_len,
                                        &sndrcvinfo, &flags);
      if (recvmsg_result == -1) {
        NFAPI_TRACE(NFAPI_TRACE_INFO, "Failed to read sctp message size errno:%d\n", errno);
      } else {
        // print the received message
        printf("\n VNF MESSAGE RECEIVED: \n");
  ...
  if (1) {
...

    {
      ...
      if (unpack_result < 0) {
        ...
        NFAPI_TRACE(NFAPI_TRACE_INFO, "VNF Unpack p5 message header success\n");
      }
      message_size = header.message_length + header_buffer_size;
...
static int vnf_send_p5_msg(nfapi_vnf_pnf_info_t *pnf, const void *msg, int len, uint8_t stream) {
  ...
  int result = sctp_sendmsg(pnf->p5_sock, msg, len, (struct sockaddr *) &pnf->p5_pnf_sockaddr,
                            sizeof(pnf->p5_pnf_sockaddr), 1, 0, stream, 0, 4);
...
int vnf_nr_pack_and_send_p5_message(vnf_t *vnf, uint16_t p5_idx, nfapi_p4_p5_message_header_t *msg, uint16_t msg_len) {
  ...
  if (pnf) {
    ...
    if (NFAPI_MODE == NFAPI_MODE_AERIAL) {
      ...
      packedMessageLength = nfapi_nr_p5_message_pack(msg, msg_len, vnf->tx_message_buffer,
...
int vnf_pack_and_send_p5_message(vnf_t *vnf, uint16_t p5_idx, nfapi_p4_p5_message_header_t *msg, uint16_t msg_len) {
  ...
  if (pnf) {
    // pack the message for transmission
    int packedMessageLength = nfapi_p5_message_pack(msg, msg_len, vnf->tx_message_buffer,
                                                    sizeof(vnf->tx_message_buffer), &vnf->_public.codec_config);
    ...
    if (msg->message_id == NFAPI_NR_PHY_MSG_TYPE_CONFIG_REQUEST) {
      ...
      printf(" msg id : 0x%02x num_tlvs %d \n", msg->message_id, pNfapiMsg->num_tlv);
      test_nv_ipc_send_msg_P5(vnf->tx_message_buffer, packedMessageLength, msg);
    }
...
int vnf_pack_and_send_p4_message(vnf_t *vnf, uint16_t p5_idx, nfapi_p4_p5_message_header_t *msg, uint16_t msg_len) {
  ...
  if (pnf) {
    // pack the message for transmission
    int packedMessageLength = nfapi_p4_message_pack(msg, msg_len, vnf->tx_message_buffer,
                                                    sizeof(vnf->tx_message_buffer), &vnf->_public.codec_config);
