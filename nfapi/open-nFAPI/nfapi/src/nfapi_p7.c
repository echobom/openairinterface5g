...
static uint8_t pack_dl_tti_csi_rs_pdu_rel15_value(void *tlv, uint8_t **ppWritePackedMsg, uint8_t *end) {
  nfapi_nr_dl_tti_csi_rs_pdu_rel15_t *value = (nfapi_nr_dl_tti_csi_rs_pdu_rel15_t *)tlv;
    ...
    // TODO Add Precoding and Beamforming
...
static uint8_t pack_dl_tti_pdcch_pdu_rel15_value(void* tlv, uint8_t **ppWritePackedMsg, uint8_t *end) {
	...
	for(uint16_t i = 0; i < value->numDlDci; ++i)//MAX_DCI_CORESET
	{
...
static uint8_t pack_dl_tti_pdsch_pdu_rel15_value(void *tlv, uint8_t **ppWritePackedMsg, uint8_t *end) {
  ...
  // TODO: resolve the packaging of array (currently sending a single element)
    ...
    for (int i = 0; i < value->NrOfCodewords; ++i) {
        push16(value->targetCodeRate[i],ppWritePackedMsg,end);
        push8(value->qamModOrder[i],ppWritePackedMsg,end);
        push8(value->mcsIndex[i],ppWritePackedMsg,end);
        push8(value->mcsTable[i],ppWritePackedMsg,end);
        push8(value->rvIndex[i],ppWritePackedMsg,end);
        push32(value->TBSize[i],ppWritePackedMsg,end);
    }
    ...
    // TODO Add TX power info
      //push dos valores que representam 0db
      push8(8,ppWritePackedMsg,end); // powerControlOffset
...
static uint8_t pack_ul_tti_request_prach_pdu(nfapi_nr_prach_pdu_t *prach_pdu, uint8_t **ppWritePackedMsg, uint8_t *end) {
    ...
    return (push16(prach_pdu->phys_cell_id, ppWritePackedMsg, end)&&
    ...
    //for (int i = 0; i < prach_pdu->beamforming.num_prgs; ++i) {
    //    push16(prach_pdu->beamforming.prgs_list[i].,ppWritePackedMsg,end);
    //    for (int k = 0; k < prach_pdu->beamforming.dig_bf_interfaces; ++k) {
    //        push16(prach_pdu->beamforming.prgs_list[i].dig_bf_interface_list[k].beam_idx,ppWritePackedMsg,end);
    //    }
    //}
    return 1;
}
