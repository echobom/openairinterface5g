...
void nr_rx_sdu(const module_id_t gnb_mod_idP,
  ...
  if (UE_id != -1) {
    ...
    for (int i = 0; i < NR_NB_RA_PROC_MAX; ++i) {
      ...
      if (ra->msg3_round >= gNB_mac->harq_round_max - 1) {
...
void pf_ul(module_id_t module_id,
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    ...
    sched_pusch->mcs = get_mcs_from_bler(nrmac->harq_round_max, bo, stats, &sched_ctrl->ul_bler_stats, max_mcs, frame);
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    ...
    /* Statistics */
    AssertFatal(cur_harq->round < nr_mac->harq_round_max, "Indexing ulsch_rounds[%d] is out of bounds\n", cur_harq->round);
