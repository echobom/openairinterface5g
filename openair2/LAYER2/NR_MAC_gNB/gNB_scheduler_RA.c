  ...
  if (ra->Msg4_frame == frameP && ra->Msg4_slot == slotP ) {
    ...
    if (harq->round == 0) {
      if (ra->msg3_dcch_dtch) {
        ...
        uint16_t mac_sdu_length = mac_rrc_nr_data_req(module_idP, CC_id, frameP, CCCH, ra->rnti, 1, &buf[mac_pdu_length + sizeof(NR_MAC_SUBHEADER_SHORT)]);
        if (mac_sdu_length == 0) {
          LOG_I(NR_MAC,"No Msg4, check DCCH message for RRCReestablishment 0x%0xx\n",ra->rnti);
          ...
          if (sched_ctrl->rlc_status[DL_SCH_LCID_DCCH].bytes_in_buffer > 0) {
            mac_sdu_length = mac_rlc_data_req(module_idP,
                                        ...
                                        (char *)&buf[mac_pdu_length + sizeof(NR_MAC_SUBHEADER_SHORT)],
                                        0,
                                        0);
          } else {
        ...
        AssertFatal(mac_sdu_length<256,"Msg4 with NR_MAC_SUBHEADER_LONG not implemented! Msg4 size: %d\n", mac_sdu_length);
