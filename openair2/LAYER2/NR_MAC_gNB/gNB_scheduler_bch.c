          ...
          for (int i_ssb=0; i_ssb<64; i_ssb++) {
            if ((longBitmap->buf[i_ssb/8]>>(7-(i_ssb%8)))&0x01) {
              ...
              if ((ssb_start_symbol/14) == rel_slot){
                ...
                const int nr_slots_period = tdd ? n_slots_frame/get_nb_periods_per_frame(scc->tdd_UL_DL_ConfigurationCommon->pattern1.dl_UL_TransmissionPeriodicity) :
                                                  n_slots_frame/get_nb_periods_per_frame(gNB->temp_dl_UL_TransmissionPeriodicity);
